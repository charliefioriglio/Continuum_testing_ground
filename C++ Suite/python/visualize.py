...
def plot_slice(data, axis, index, title):
...
def plot_isosurface(data, spacing, origin, isovalue=0.02):
    # Data is (nx, ny, nz)
    # Origin is (x0, y0, z0)
    # Spacing is (dx, dy, dz)
...
if __name__ == "__main__":
    ...
    if args.slice:
        ...
        plot_slice(data, args.axis, idx, f"Slice Axis {args.axis} Index {idx}")
    else:
        print(f"Generating 3D isosurface at +/- {args.isovalue}...")
        try:
            plot_isosurface(data, spacing, origin, args.isovalue)
