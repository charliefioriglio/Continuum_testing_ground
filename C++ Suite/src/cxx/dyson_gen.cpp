...
int main(int argc, char** argv) {
    ...
    */
    ...
    // Look for --point-dipole D and --lmax L
    bool use_point_dipole = false;
    double dipole_strength_input = 0.0; // The D value
    int override_lmax = -1;
    ...
    for(int i=3; i<argc; ++i) {
        ...
        if (arg == "--point-dipole" && i+1 < argc) {
             ...
             try { dipole_strength_input = std::stod(argv[++i]); } catch(...) {}
        }
        if (arg == "--lmax" && i+1 < argc) {
             try { override_lmax = std::stoi(argv[++i]); } catch(...) {}
        }
    }

    if (in >> ie_ev >> input_l_max >> n_pts >> e_min >> e_max) {
        ...
        // if(use_point_dipole) std::cerr << "Using Point Dipole Model (D=" << dipole_strength_input << ")" << std::endl;
        ...
        if (use_point_dipole) {
             xs = CrossSectionCalculator::ComputePointDipoleCrossSection(
                L, R, grid, energies, ie_ev, l_max, dipole_strength_input
            );
        } else {
