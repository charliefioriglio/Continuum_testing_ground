...
# ---------------------------------------------
# Define Grid (identical to CN.py)
# ---------------------------------------------
L = 10
n_pts = 100
...
# ---------------------------------------------
# Orientational averaging routine (identical to CN.py logic)
# ---------------------------------------------
def compute_average_amplitudes(X, Y, Z, DO_coeffs_L, DO_coeffs_R, ang_grid, k_lab, pol_lab):
    from CuO_do import build_DO
...
"""Main calculation wrapper with generic handling of degeneracies."""


def calculate_beta(E_eV, ang_grid, degeneracies=None, pol_lab=None):
    """Compute beta(E) allowing for an arbitrary set of degenerate channels.

    Parameters
    ----------
    E_eV : array_like
        Photoelectron kinetic energies in eV.
    ang_grid : ndarray, shape (3, N)
        Euler angle grid.
    degeneracies : sequence of (DO_L, DO_R) pairs, optional
        Each element is a tuple (DO_coeffs_L, DO_coeffs_R) defining one
        Dyson orbital channel. All channels are summed incoherently.
        If None, defaults to the two CuO b1/b2 channels.
    pol_lab : array_like, optional
        Polarization vector in lab frame. Defaults to z-axis.
    """
    ...
    for E in E_eV:
        ...
        k_par_scaled = k_par_lab * k_mag
        k_perp1_scaled = k_perp1_lab * k_mag
        k_perp2_scaled = k_perp2_lab * k_mag
        ...
        for DO_L, DO_R in degeneracies:
            A_par = compute_average_amplitudes(
                X, Y, Z, DO_L, DO_R, ang_grid, k_par_scaled, pol_lab
            )
            A_perp1 = compute_average_amplitudes(
                X, Y, Z, DO_L, DO_R, ang_grid, k_perp1_scaled, pol_lab
            )
            A_perp2 = compute_average_amplitudes(
                X, Y, Z, DO_L, DO_R, ang_grid, k_perp2_scaled, pol_lab
            )
...
if __name__ == "__main__":
    ...
    # Angular grid copied from CN.py
    print("Loading angular grid...")
    ang_grid = np.array([
    ...
    # Generate Euler angle grid using repulsion algorithm
    #n_orientations = 50  # Change this to adjust number of angles
    #print(f"Generating angular grid with {n_orientations} orientations...")
    ...
    # Beta vs energy (beta is in column index 3)
    ...
    plt.title('Anisotropy Parameter', fontsize=13)
    plt.ylim(-1, 2)
