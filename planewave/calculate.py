...
except ImportError:  # optional helper, keep going if unavailable
...
# ---------------------------------------------
# Global grid definition (matches beta.py defaults)
# ---------------------------------------------
L = 10
n_pts = 100
...
# ---------------------------------------------
def calculate_relative_cross_sections(E_photon_grid, Trans_E, FC_factors, x, y, z, DO_coeffs_L, DO_coeffs_R, L_max):
...
def calculate_total_cross_sections(E_photon_grid, Trans_E, x, y, z, DO_coeffs_L, DO_coeffs_R, DO_L_norm, DO_R_norm, L_max):
    DO_L = build_DO(DO_coeffs_L, x, y, z, dV)
    DO_R = build_DO(DO_coeffs_R, x, y, z, dV)
...
rel_cross_sections = calculate_relative_cross_sections(E_photon_grid, Trans_E, FC_factors, X, Y, Z, DO_coeffs_a1_L, DO_coeffs_a1_R, L_max)

# Save relative cross sections to CSV
import pandas as pd
df = pd.DataFrame({
    'E_photon': E_photon_grid,
    'v=0': rel_cross_sections[:, 0],
    'v=1': rel_cross_sections[:, 1],
    'v=2': rel_cross_sections[:, 2],
    'v=3': rel_cross_sections[:, 3],
})
df.to_csv('relative_cross_sections.csv', index=False)
print("Saved relative cross sections to 'relative_cross_sections.csv'")