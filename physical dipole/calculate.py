...
from build_mode_num import build_single_mode_wavefunction_on_xyz_grid
...
# ---------------------------------------------
def calculate_cross_sections(E_photon_grid, Trans_E, FC_factors, x, y, z, DO_coeffs_L, DO_coeffs_R, DO_L_norm, DO_R_norm, L_max, D, a):
            ...
            for pol in [(1, 0, 0), (0, 1, 0), (0, 0, 1)]:
                for m in range(-L_max, L_max + 1):
                    ...
                    for n in range(L_max + 1 - abs(m)):
                        ...
                        psi_el = build_single_mode_wavefunction_on_xyz_grid(m, n, eKE+1e-10, a, D, x, y, z)
                        if psi_el is None:
                            continue
                        dipole = pol[0]*x + pol[1]*y + pol[2]*z
...
def calculate_total_cross_sections(E_photon_grid, Trans_E, x, y, z, DO_coeffs_L, DO_coeffs_R, DO_L_norm, DO_R_norm, L_max, D, a):
    DO_L = build_DO(DO_coeffs_L, x, y, z, dV)
    DO_R = build_DO(DO_coeffs_R, x, y, z, dV)
...
#for D in Dipoles:
#    rel_cross_sections = calculate_cross_sections(E_photon_grid, Trans_E, FC_factors, X, Y, Z, DO_coeffs_b1_L, DO_coeffs_b1_R, b1_L_norm, b1_R_norm, L_max, D)
...
# Print the columns
for D in Dipoles:
    total_cross_sections = calculate_total_cross_sections(E_photon_grid, Trans_E[0], X, Y, Z, DO_coeffs_b1_L, DO_coeffs_b1_R, b1_L_norm, b1_R_norm, L_max, D, a)
